...
plot(x=dat$Species, y=dat$Petal.Length)
plot(x=dat$Species, y=dat$Sepal.Width)
plot(x=dat$Species, y=dat$Petal.Width)
...
evenrows <- seq(2,150,2) #taking the number of rows
print(evenrows)
iris_even_numbered_rows <- iris[evenrows,0]
print(iris_even_numbered_rows)
# 2.  Create a new object called iris_chr which is a copy of iris, except where every column is a character class

iris_chara <- data.frame(lapply(iris, as.character), stringsAsFactors = FALSE)
print(iris_chara)
...
#this makes iris$Sepal.Area an object of iris(or child of iris)
